...
async def support_(message: types.Message): # В параметры функции попадает событие сообщения из чата
    await bot.send_sticker(message.from_user.id, sticker='CAACAgIAAxkBAAEJZDpkjyiCq7JzLczhnaE7yRp7noNrawACQx0AAmt1-UkOzn3LS4oOBi8E') # await ждёт пока в потоке появится свободное место
    await message.delete() # await ждёт пока в потоке появится свободное место
